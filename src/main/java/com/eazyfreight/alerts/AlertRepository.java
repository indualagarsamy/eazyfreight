...
public interface AlertRepository extends JpaRepository<Alert, UUID> {
    ...
    @Query("""
            ...
            where a.bookingId = :bookingId and a.alertType = :alertType
              and a.status <> com.eazyfreight.alerts.AlertStatus.RESOLVED
            """)
    ...
    @Query("""
            select a from Alert a
            where a.status <> com.eazyfreight.alerts.AlertStatus.RESOLVED
    ...
    @Query("""
            select a from Alert a
            where a.status = com.eazyfreight.alerts.AlertStatus.SNOOZED
    ...
    @Query("""
            select a from Alert a
            where a.status = com.eazyfreight.alerts.AlertStatus.ACTIVE
            """)
