...
This is the Windows-specific companion to [README.md](README.md); see that file for
everything not related to environment setup.
...
Commands below are PowerShell. Java 21 is required. If it isn't already on your
`PATH` (e.g. installed via the [Adoptium MSI](https://adoptium.net/) or
`winget install EclipseAdoptium.Temurin.21.JDK`), point `JAVA_HOME` at it for the
current session:
...
(Use `setx JAVA_HOME "C:\Program Files\Eclipse Adoptium\jdk-21"` to persist it across
sessions — open a new terminal afterwards for it to take effect.)

```powershell
.\gradlew.bat build            # compiles and runs all 65 tests; no docker needed
```

```powershell
docker compose up -d           # postgres on 5432, jaeger UI on 16686
.\gradlew.bat bootRun           # service on 8080
